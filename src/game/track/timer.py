...
class Timer:
  ...
  def draw_timer(self, x: int, y: int):
    ...
    pr.draw_text(total_time, x, y, 30, pr.BLACK)
    pr.draw_text(self.prev_lap_time.str_format, x, y + 70, 20, pr.BLACK)
    pr.draw_text(self.best_lap_time.str_format, x, y + 120, 20, pr.BLACK)

    for i in range(3):
      pr.draw_text(self.sector_times[i].str_format, x + 100 * i, y + 30, 20, pr.BLACK)
      pr.draw_text(
        self.prev_sector_times[i].str_format, x + 100 * i, y + 90, 20, pr.BLACK
      )
      pr.draw_text(
        self.best_sector_times[i].str_format, x + 100 * i, y + 140, 20, pr.BLACK
      )
